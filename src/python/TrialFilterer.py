...
class TrialFilterer:
    ...
    def filter_keywords(
        ...
        if info['numExacerbations'] > 0:
            df.loc[df['Keyword'].str.contains('|'.join(['exacerbation','Exacerbation']),na=False), ['KeywordRank']] += 1
        if info['numFlares'] > 0:
            df.loc[df['Keyword'].str.contains('|'.join(['flare','flare up','flare-up','Flare','Flare up','Flare-up']),na=False), ['KeywordRank']] += 1
        if info['usesInhaler']:
            df.loc[df['Keyword'].str.contains('|'.join(['inhaler', 'Inhaler']),na=False), ['KeywordRank']] += 1
        if info['usesInjection']:
            df.loc[df['Keyword'].str.contains('|'.join(['injection', 'Injection']),na=False), ['KeywordRank']] += 1
        if info['isSmoker']:
            df.loc[df['Keyword'].str.contains('|'.join(['smoker', 'smoking', 'Smoker', 'Smoking']),na=False), ['KeywordRank']] += 1
        if info['asthmaSeverity'] in ["moderate", "severe"]:
            df.loc[df['Keyword'].str.contains('|'.join(['severe asthma', 'moderate asthma', 'Severe asthma', 'Moderate asthma']),na=False), ['KeywordRank']] += 1
        if info["isEosinophilic"]:
            df.loc[df['Keyword'].str.contains('|'.join(['Eosinophilic','eosinophilic','Eosinophilia','eosinophilia']),na=False), ['KeywordRank']] += 1
        if info["usesPillsTablets"]:
            df.loc[df['Keyword'].str.contains('|'.join(['pill','Pill','tablet','Tablet','oral','Oral']),na=False), ['KeywordRank']] += 1
        if info["onDualTherapy"]:
            df.loc[df['Keyword'].str.contains('|'.join(['dual therapy','Dual therapy']),na=False), ['KeywordRank']] += 1
        if info["onTripleTherapy"]:
            df.loc[df['Keyword'].str.contains('|'.join(['triple therapy','Triple therapy']),na=False), ['KeywordRank']] += 1
        if info["numCOPDFlares"] > 0:
            df.loc[df['Keyword'].str.contains('|'.join(['COPD flare', 'COPD Flare']),na=False), ['KeywordRank']] += 1
        return df
