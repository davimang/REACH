...
and technologies that can/will be used to test the software system being built {\citet{DevPlan}}. Additionally, the documentation detailing the potential faults and their impacts can be found in the hazard analysis {\citet{HazardAnalysis}}. Finally, the MIS document which details the modular design 
...
Testing for maintainability will be conducted by at least one other member of the team. Once a member opens a pull request, another member of the team will independently review the code. In addition to the normal functional testing, they will conduct a code review to check for the maintainability of the code, specifically checking the code structuring and flow to ensure it meets a high standard of maintainability.
...
Input: The user selects the sign-out button.
...
Input: The user clicks the trial result.
...
Input: The uses clicks the FAQ.
					
Output: FAQ menu page.
...
Input: No search fields are filled out.
					
Output: Message informing user of failure.
...
Input: The users clicks the option to bookmark a trial.
					
Output: The user interface gives feedback indicating that it has been saved.
...
Input/Condition: The user has populated any number of the search fields with valid values.
...
How test will be performed: Manually run linters on code before merging a PR. The test is considered to pass if the linter does not return any errors, and is considered to have failed if it does.
...
Initial State: The user is logged in, and is at the search screen.

Input: User search criteria - the set of health parameters a given user wants to search with.

Output: Display providing the user info regarding data collection and consent.

Test Case Derivation: From NFR-18, regarding the collection of user consent before using or storing user data.

How test will be performed: Users are asked to input their search criteria, and should see a message prompting them to provide consent for the collection of data when they try to search. If the user sees the message, the test passes.
